data "archive_file" "lambda-get-zip" {
    ...
    source_file = "C:\\Users\\tone.herndon\\Git\\back-end\\lambda_get_function.py"
}
...
resource "aws_lambda_function" "lambda-get-function" {
    ...
    function_name       = "cloud-resume-challenge-lambda-get"
...
resource "aws_iam_policy" "get-dynamodb-policy" {
    name        = "cloud-resume-challenge-lambda-get-dynamodb-policy"
    description = "Policy for cloud-resume-challenge-lambda-get-role to manage counter-table dynamodb"
    policy      = <<POLICY
{
    "Version": "2012-10-17"
    "Statement": [
...
resource "aws_iam_policy" "lambda-get-logging" {
    name        = "cloud-resume-challenge-lambda-get-logging-policy"
    path        = "/"
